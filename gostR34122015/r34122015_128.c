...
uint8_t K_POS[256] =
{
	...
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 
	0x05, 0x00, 0x06, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 
...
void R128inv(BLOCK128 a0)
{
	...
	memmove(a0 + 1, a0, BLOCK128SIZE - 1);
	a0[0] = tmp;
	tmp = 0;
	for(size_t i = 0; i < BLOCK128SIZE; i++)
	{
...
{
	BLOCK128 tmp = {0};
	memcpy(tmp, a0, BLOCK128SIZE);

	X128(a0, c, a0);
	S128(a0);
	L128(a0);
	X128(a0, a1, a0);
	memcpy(a1, tmp, BLOCK128SIZE);
...
{
	BLOCK128 tmp = {0};
	memcpy(K128[0], pwd + BLOCK128SIZE, BLOCK128SIZE);
	memcpy(K128[1], pwd, BLOCK128SIZE);

	for(size_t i = 1; i < 5; i++)
	{
		memcpy(K128[2 * i + 1], K128[2 * i - 2], BLOCK128SIZE);
		memcpy(K128[2 * i], K128[2 * i - 1], BLOCK128SIZE);
		for(size_t j = 0; j < 8; j++)
		{
			F128(K128[2 * i + 1], K128[2 * i], C128[8 * (i - 1) + j]);
		}
		memcpy(tmp, K128[2 * i + 1], BLOCK128SIZE);
		memcpy(K128[2 * i + 1], K128[2 * i], BLOCK128SIZE);
		memcpy(K128[2 * i], tmp, BLOCK128SIZE);
	}
	memset(tmp, 0x00, BLOCK128SIZE);
}
